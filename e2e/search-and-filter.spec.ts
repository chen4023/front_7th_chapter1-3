...
test.describe('검색 및 필터링 E2E 테스트', () => {
  ...
  test('제목으로 일정을 검색할 수 있다', async ({ page }) => {
    ...
    await expect(
      list.getByText(`${hours}:${minutes} - ${endHours}:${minutes}`).first()
    ).toBeVisible({
      timeout: 10000,
    });
    ...
    await page.getByLabel('종료 시간').fill(`${endHours2}:${minutes2}`);
    ...
    // 두 일정 생성 확인
    await expect(
      list.getByText(`${hours2}:${minutes2} - ${endHours2}:${minutes2}`).first()
    ).toBeVisible({
      timeout: 10000,
    });
